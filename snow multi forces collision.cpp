...
void generateObjectBufferTeapot(GLuint shaderProgramID) {
	...
	for (int i = 0; i < PARTICLE_COUNT; i++) {
		...
		//half sphere emitter (from a cloud)
		// + /-
		...
		float radius = (float)(rand() % 60);
		float speed = ((float)(rand() % 7)) / 10.0f;

		float sInclination = sinf(inclination);

		float x = sInclination * cosf(inclination);
		float y = -1.0 * sInclination * sinf(inclination); // snow naturally falling down...
...
{
	// Set up the shaders
	shaderParticles = CompileShaders(shaderParticles, "../Shaders/snowVS.txt", "../Shaders/snowFS.txt");
